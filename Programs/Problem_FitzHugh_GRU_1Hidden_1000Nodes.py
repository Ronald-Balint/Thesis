...
dv_dt = tf.cast(d_dt(predict0[:,0]),tf.float64)
dw_dt = tf.cast(d_dt(predict0[:,1]),tf.float64)

  
#Setting up NN inputs and Outputs for Parameter Estimation    
vR=tf.cast(predict0[:,0],tf.float64)
wR=tf.cast(predict0[:,1],tf.float64)


#Defining Custom Loss Function  
def my_loss_fn(y_true, y_pred):
    a= tf.cast(y_pred[0,0],tf.float64)
    b= tf.cast(y_pred[0,1],tf.float64)
    c= tf.cast(y_pred[0,2],tf.float64)
    cube=tf.cast(np.full(len(v),3),tf.float64)
    ick_1rhs = math.multiply(math.reciprocal(c),math.subtract(math.subtract(vR,math.divide(math.pow(vR,cube),tf.cast(3,tf.float64))),wR))
...
x = layers.GRU(1000, activation='tanh', return_sequences=True, dtype='float64')(inputs)
outputs = layers.GRU(3, activation='linear', dtype='float64')(x)
