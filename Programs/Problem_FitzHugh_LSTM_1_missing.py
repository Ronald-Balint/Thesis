...
#Setup dataset
whole_dataset = K.utils.timeseries_dataset_from_array(
    train
    ,targets=again
    ,sequence_length=1
    ,sampling_rate=1
...
outputs = layers.LSTM(3, activation='linear')(inputs)
...
history = modele.fit(whole_dataset, 
           epochs=9,
           verbose=2,
           #callbacks=[ bestmodel]
           )
...
par = modele.predict(whole_dataset)
...
plt.plot(t,v_sol,"b-",label="Generated v")
plt.plot(t,w_sol,"g-",label="Generated w")
plt.plot(t,predict0[:,0],"k-", label="v NN Prediction")
plt.plot(t,predict0[:,1],"k-", label="w NN Prediction")
plt.legend()
