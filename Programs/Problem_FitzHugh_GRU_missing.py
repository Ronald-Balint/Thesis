...
def my_loss_fn(y_true, y_pred):
    ...
    sv = v[4:19]
    sw = w[4:19]
    ick_1rhs = math.multiply(math.reciprocal(c),math.subtract(math.subtract(sv,math.divide(math.pow(sv,cube),3)),sw))
    ick_1 = math.subtract(dv,ick_1rhs)
    
    ick_2rhs = math.multiply(c,(math.add(math.subtract(sv,math.multiply(a,sw)),b)))
    ick_2 = math.subtract(dw,ick_2rhs)
...
x = layers.GRU(60, activation='sigmoid',return_sequences=True)(inputs)
