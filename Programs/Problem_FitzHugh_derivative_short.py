...
val_dataset = K.utils.timeseries_dataset_from_array(
    this_wont_work[10:15],
    targets=np.ones((5,1)),
    sequence_length=1,
    shuffle=True,
    batch_size=1)

test_dataset = K.utils.timeseries_dataset_from_array(
    this_wont_work[15:],
    targets=np.ones((5,1)),
    sequence_length=1,
    shuffle=True,
...
x = layers.Dense(3, activation='tanh', return_sequences=True, dtype="float64")(inputs)
outputs = layers.Dense(3, activation='linear', dtype="float64", activity_regularizer='l1')(x)
...
par = modele.predict(test_dataset)
...
epochs = range(int(len(loss)/2), len(loss) + 1)
plt.figure()
plt.plot(epochs, loss, "o", color=3)
