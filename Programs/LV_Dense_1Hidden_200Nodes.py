...
initializer = K.initializers.RandomUniform(minval=.5, maxval=1.5, seed=20)

opt = K.optimizers.RMSprop(learning_rate=0.01)
#opt = K.mixed_precision.LossScaleOptimizer(opt)

modele = K.Sequential([
    Dense(200, activation="sigmoid", kernel_initializer=initializer)
    ,Dense(4,activation="relu", dtype='float64', kernel_initializer=initializer)
    ])
